...
contract RiscZeroGroth16Verifier is IRiscZeroVerifier, Groth16Verifier {
    ...
    function verifySnark(bytes memory seal, bytes32 imageId, bytes32 postStateDigest, bytes calldata journal)
        public
        view
        returns (bool)
    {
        return verify(seal, imageId, postStateDigest, journal);
    }
}