...
describe('W3bstream Router', function () {
  ...
  beforeEach(async function() {
    ...
    dummyProcessor = await ethers.deployContract('DummyProcessor');
    dummyMSP = await ethers.deployContract('DummyMSP');
  ...
  it('route', async function() {
    ...
    await fleetManagement.setMSP(dummyMSP.getAddress());
    ...
    await w3bstreamRouter.connect(projectOwner).bindDapp(projectId, dummyProcessor.getAddress());
